...
import { MENU_CATEGORIES, MenuItem } from "@/lib/data/menuData";
    ...
    return items.filter((item) => {
      ...
      const matchesCategory =
        selectedCategory === "semua"
          ? searchQuery
            ? true
            : generalCategories.includes(item.category)
    ...
    <div className="w-full bg-[#fdfbfc] text-[#241b18] min-h-screen pt-28 pb-20">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center justify-between mb-6 text-xs text-[#665b56]">
          <span>
            Menampilkan <strong className="text-[#241b18]">{filteredItems.length}</strong> menu
          </span>
          {totalItems > 0 && (
            <button
              onClick={toggleCart}
